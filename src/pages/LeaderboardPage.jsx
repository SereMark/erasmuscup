...
export default function LeaderboardPage() {
  ...
  return (
    <section className="min-h-screen bg-[#121212] text-white pb-12 overflow-hidden">
      <div className="relative w-full h-[50vh] overflow-hidden">
        ...
        <div className="relative w-full h-full flex items-center justify-center">
          <motion.div
            ...
            className="px-6 max-w-3xl text-center backdrop-blur-sm bg-black/20 p-8 rounded-2xl border border-white/10"
          >
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight mb-6 text-transparent bg-clip-text bg-gradient-to-br from-white to-gray-400 drop-shadow-lg">
              Who Rules Them All?
            </h1>
      ...
      <div className="px-4 sm:px-6 max-w-6xl mx-auto -mt-16 mb-16 relative z-10">
        <motion.div
        ...
        >
          {sortedHouseTotals.slice(0, 3).map((house, index) => {
            const rank = index + 1;
            const podiumHeight = rank === 1 ? "h-64" : rank === 2 ? "h-56" : "h-48";
            ...
            return (
              ...
              >
                <div className={`bg-gradient-to-br ${getHouseColorClass(house.key)} p-1 rounded-2xl shadow-[0_15px_30px_rgba(0,0,0,0.4)]`}>
                  <div className="bg-[#1a1a1a]/90 backdrop-blur-sm rounded-2xl p-6 flex flex-col items-center">
                    <motion.div
                    ...
                    <motion.div
                      ...
                      className="w-16 h-16 mb-4 bg-gradient-to-br from-gray-800 to-gray-900 rounded-full p-1 shadow-lg"
                    >
                      <img 
                        src={house.logo} 
                    ...
                    <h3 className="text-xl font-bold mb-1">{house.label}</h3>
                    <div className="text-4xl font-black mb-3 text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400">
                      {house.points}
                    </div>
                    <div className="text-sm text-gray-400 font-semibold">Rank #{rank}</div>
                    <motion.div
                      initial={{ height: 0 }}
                      animate={{ height: "auto" }}
                      transition={{ duration: 0.7, delay: delay + 0.3 }}
                      className={`w-full mt-4 bg-gradient-to-t ${getHouseColorClass(house.key)} rounded-lg opacity-20`}
      ...
      <div className="px-4 sm:px-6 max-w-6xl mx-auto">
        ...
        >
          <motion.h2
            ...
            className="text-3xl sm:text-4xl font-extrabold mb-4 sm:mb-0 text-center tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-purple-300 to-pink-300"
          >
            Scoreboard Details
          </motion.h2>
          <motion.div
              ...
              {scoreboardData.map((row, index) => {
                ...
                return (
                  <motion.div
                    ...
                    viewport={{ once: true, margin: "-100px" }}
                    transition={{ duration: 0.5, delay: index * 0.07 }}
                    className={`p-6 rounded-xl shadow-xl backdrop-blur-md ${cardBgClass}`}
                  >
                    <div className="mb-4 flex justify-between items-start">
                      <div>
                        {row.adjustment && !row.total && (
                          <span className="inline-block bg-purple-900/30 text-purple-300 text-xs px-2 py-0.5 rounded-md mb-2">
                            ADJUSTMENT
                          </span>
                        )}
                        <h3 className={`text-xl ${row.total ? "text-2xl font-bold text-purple-200" : "font-medium text-white"}`}>
                          {eventDisplay}
                        </h3>
                        {dateDisplay && (
                          <p className="text-sm text-gray-400 font-mono mt-1">{dateDisplay}</p>
                        )}
                      </div>
                      {row.total && (
                        <div className="flex -space-x-2">
                          {[...Array(3)].map((_, i) => (
                            <div 
                              key={i} 
                              className={`w-7 h-7 rounded-full flex items-center justify-center bg-purple-600 ${
                    ...
                    <div className="grid grid-cols-2 gap-3 mt-4">
                      {sortedRowHouses.map(house => {
                        ...
                        return (
                          ...
                          >
                            <div className="w-8 h-8 bg-black/30 rounded-full p-0.5 mr-3">
                              <img 
                                src={house.logo} 
                                alt={`${house.label} logo`}
                                className="w-full h-full object-contain"
                            ...
                            <div>
                              <span className="text-xs block">{house.label}</span>
        ...
        >
          <p className="text-center text-gray-400 text-sm leading-relaxed max-w-2xl mx-auto bg-black/30 p-6 rounded-xl backdrop-blur-sm">
