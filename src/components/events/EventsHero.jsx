...
export default function EventsHero({ currentEvent, defaultHero }) {
  ...
  const badgeVariants = {
    ...
    visible: { opacity: 1, scale: 1, transition: { delay: 0.4, duration: 0.6, ease: [.43, .13, .23, .96] } },
    pulse: { scale: [1, 1.05, 1], transition: { duration: 2, repeat: Infinity, repeatType: "loop" } }
  }
  ...
  return (
    <section className="relative w-full h-[60vh] min-h-[500px] overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <motion.div
          className="absolute top-1/4 left-1/4 w-[35rem] h-[35rem] rounded-full bg-brand-500/10 blur-[100px] opacity-60"
          animate={{ scale: [1, 1.05, 1], opacity: [0.6, 0.4, 0.6] }}
          transition={{ duration: 8, repeat: Infinity, repeatType: "reverse" }}
        />
        <motion.div
          className="absolute bottom-1/3 right-1/3 w-[30rem] h-[30rem] rounded-full bg-accent-400/10 blur-[100px] opacity-70"
