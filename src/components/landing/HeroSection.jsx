  ...
  return (
    <section ref={heroRef} className="relative w-full h-screen overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-1/4 left-1/4 w-[40rem] h-[40rem] rounded-full bg-brand-500/10 blur-[100px] animate-pulse-slow transform -translate-x-1/2 -translate-y-1/2" />
        <div className="absolute bottom-1/3 right-1/3 w-[30rem] h-[30rem] rounded-full bg-accent-400/10 blur-[100px] animate-pulse-slow transform translate-x-1/2 translate-y-1/2 animation-delay-1000" />
      </div>
      <div className="relative h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-center items-center text-center">
        <motion.div initial="hidden" animate="visible" style={{ scale }} className="space-y-6 w-full">
          <motion.h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-bold text-balance mx-auto" variants={titleVariants}>
          ...
          <motion.p className="text-xl sm:text-2xl text-gray-300 max-w-3xl mx-auto font-light" variants={subtitleVariants}>
            {data.subtitle}
          </motion.p>
          <motion.div className="flex flex-col sm:flex-row gap-4 pt-8 justify-center items-center" variants={subtitleVariants}>
            {data.buttons.map((button, i) => (
              <motion.div key={i} variants={buttonVariants} custom={i} whileHover="hover" whileTap="tap">
                <Link
                  to={button.link}
                  className={`inline-flex items-center justify-center px-8 py-4 rounded-full text-lg font-medium transition-all duration-300 ${
