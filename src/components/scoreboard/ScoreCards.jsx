          ...
          >
            <div className="px-4 sm:px-6 pt-4 sm:pt-6 pb-3 sm:pb-4 border-b border-white/5">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 sm:gap-0">
                <div>
                  <h3 className="text-lg sm:text-xl font-semibold text-white group-hover:text-brand-300 transition-colors duration-300">
                    {event.eventName}
                  </h3>
                </div>
                {event.date ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                {filteredHouses.map(house => (
                  <div
                    ...
                    className="flex items-center gap-2 sm:gap-3 p-2 rounded-lg bg-dark-800/20 hover:bg-dark-800/30 transition-colors duration-200"
                  >
                    ...
                    <div>
                      <div className={`text-xs sm:text-sm font-medium text-house-${house.key}`}>{house.name}</div>
                      <div
                        className={`inline-flex items-center px-2 py-0.5 sm:px-2.5 sm:py-1 mt-1 rounded-full text-xs sm:text-sm font-medium ${
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6">
              {housesFilter
                ...
                .map(house => (
                  ...
                  >
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full overflow-hidden bg-dark-800 mb-2 sm:mb-3 border border-white/10">
                      <img
                        src={house.logo}
                        alt={house.name}
                        className="w-full h-full object-cover"
                    ...
                    <div className={`text-xs sm:text-sm text-house-${house.key} mb-1`}>{house.name}</div>
