...
export default function ScoreboardHero({ data, houses, scores }) {
  ...
  return (
    <section className="relative w-full min-h-[50vh] lg:min-h-[60vh] overflow-hidden">
      ...
      >
        ...
        >
          <div className={`absolute inset-0 bg-gradient-to-b ${data.gradient || 'from-dark-950/80 via-dark-900/70 to-dark-950'}`} />
        </div>
      </motion.div>
      
      {/* Decorative glow effects */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-1/4 left-1/4 w-screen h-screen rounded-full bg-brand-500/10 blur-[100px] opacity-60" />
        <div className="absolute bottom-1/3 right-1/3 w-screen h-screen rounded-full bg-accent-400/10 blur-[100px] opacity-70 animation-delay-1000" />
      </div>
      
      {/* Content */}
      <div className="relative h-full max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-20 flex flex-col justify-center items-center text-center">
        <motion.div initial="hidden" animate="visible" className="space-y-4 sm:space-y-6 w-full">
          ...
          {standings.length > 0 && (
            <motion.div
              className="mt-8 sm:mt-12 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-8"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              {standings.slice(0, 3).map((house, i) => (
                <motion.div
                  ...
                  animate="visible"
                  whileHover="hover"
                  className="relative pt-6 pl-6 mb-2 sm:mb-0 w-full sm:w-auto max-w-[280px]"
                >
                  {/* Position badge */}
                  <div
                    className={`absolute -top-2 -left-2 w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center ring-4 ring-dark-900 z-20 ${
                      i === 0
